 ...
 * handle and an info button, a canvas (not markers: this is thousands of
 * stations worldwide, and this app already learned the hard way what
 * eight hundred DOM markers does to a pan), and a floating picker for how
 * each station draws.
 *
...
console.log('\n1. the overlay is wired in, end to end');
{
  ...
  ok('the toggle starts and stops the layer, and shows its own picker panel',
     /id === 'metar'\) \{\s*\n\s*if \(_metarActive\) _metarStop\(\); else _metarStart\(\);/.test(PAGE)
     && /id === 'metar'[\s\S]{0,400}metar-controls/.test(PAGE));
  ...
  ok('the picker panel exists, draggable, with its own header and legend',
     /<div id="metar-controls">/.test(PAGE)
     && /id="metar-drag" title="Drag to move"/.test(PAGE)
     && /_makeDraggable\(document\.getElementById\('metar-controls'\),\s*document\.getElementById\('metar-drag'\)\)/.test(PAGE));
  ok('the panel joins the shared overlay-panel shell (position, background, glass)',
     /#tornado-tracks-panel, #metar-controls \{/.test(PAGE)
     && /#tornado-tracks-panel, #metar-controls \{\s*\n\s*background-image: var\(--grad-ink\);/.test(PAGE));
...
{
  ok('the fetch asks for the real wind fields, not a guess',
...
console.log('\n7. the style choice is remembered, and every button agrees with it');
{
  ...
  ok('all four picker buttons exist, one call each',
     /_metarPickStyle\('dot'\)/.test(PAGE) && /_metarPickStyle\('temp'\)/.test(PAGE)
     && /_metarPickStyle\('barb'\)/.test(PAGE) && /_metarPickStyle\('temp-wind'\)/.test(PAGE));
}
...
console.log('\n9. live in the browser: toggling the overlay, and the declutter');
{
  ...
  const r = await p.evaluate(async () => {
    ...
    // screen at any real zoom) plus one clearly separate but still on
    // screen, so a redraw proves the crowded pair collapses to one symbol
    // while the lone station still gets its own.
    const before = { panelDisplay: getComputedStyle(document.getElementById('metar-controls')).display };
    toggleOverlayPill('metar');
    await wait(300);
    const on = {
      active: _metarActive,
      pillLit: document.getElementById('op-metar').classList.contains('active'),
      panelShown: getComputedStyle(document.getElementById('metar-controls')).display !== 'none',
    ...
    // Style picking, live.
    _metarPickStyle('temp');
    const styleOn = {
      style: _metarStyle,
      saved: localStorage.getItem('gwcfc_metar_style'),
      buttonLit: document.getElementById('metar-s-temp').classList.contains('active'),
      dotButtonOff: !document.getElementById('metar-s-dot').classList.contains('active'),
    };
    toggleOverlayPill('metar');
    await wait(150);
    const off = {
    ...
    return { before, on, declutter, styleOn, off };
  });
  ok('the picker panel starts hidden', r.before.panelDisplay === 'none', r.before.panelDisplay);
  ok('turning it on lights the pill, shows the panel and the canvas',
     r.on.active && r.on.pillLit && r.on.panelShown && r.on.canvasShown, JSON.stringify(r.on));
  ...
  ok('picking a style updates state, storage and the buttons together',
     r.styleOn.style === 'temp' && r.styleOn.saved === 'temp'
     && r.styleOn.buttonLit && r.styleOn.dotButtonOff, JSON.stringify(r.styleOn));
  ok('turning it back off drops the pill, the panel and the canvas',
     !r.off.active && !r.off.pillLit && !r.off.panelShown && !r.off.canvasShown, JSON.stringify(r.off));
