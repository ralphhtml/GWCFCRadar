...
ok('the Coral Reef Watch path is there, as the menu walks it', L.includes('Waves > SST > Coral Reef Watch > Actual'), L.filter(x => /Coral/.test(x)).join(' | '));
...
ok('any words in any order find the path', crw.some(x => x.value === 'Waves > SST > Coral Reef Watch > Actual'), crw.map(x => x.value).join(' | '));
const two = mod.completeList('layer', 'Waves > SST > Coral Reef Watch > Actual | velo');
ok('a second layer after | keeps the first', two.length > 0 && two.every(x => x.value.startsWith('Waves > SST > Coral Reef Watch > Actual | ')), two.slice(0, 2).map(x => x.value).join(' || '));
...
ok('a real path is accepted, in any case and spacing', mod.validateMapOptions({ layer: 'waves>sst>coral reef watch>ACTUAL' }).length === 0);
...
const q = mod.mapUrlParams({ place: 'plains', zoom: 7, layer: 'Waves > SST > Coral Reef Watch > Actual', overlay: spcPath });
ok('the link taps the menu path', q.get('menu') === 'Waves>SST>Coral Reef Watch>Actual', q.toString());
